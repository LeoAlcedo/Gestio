...
// helper para escapar (si lo necesitaras)
...
// Evento agregar (envía POST a guardar.php y recibe JSON)
document.getElementById("btnAgregar").addEventListener("click", function () {
    ...
    // Si estamos editando la fila (guardar cambios)
    if (filaEnEdicion) {
    ...
    .then(data => {
        ...
        fila.innerHTML = `
            ...
            <td>${escapeHtml(data.area)}</td>
            <td>${escapeHtml(data.fechaInc)}</td>
            <td>${escapeHtml(data.sintomas)}</td>
            <td>${escapeHtml(data.categoria)}</td>
            <td>${escapeHtml(data.responsable)}</td>
            <td>${escapeHtml(data.fechaRes)}</td>
            <td>${escapeHtml(data.actividadRes)}</td>
            <td>${escapeHtml(data.cierre)}</td>
        `;

        // botón eliminar flotante (no ocupa celda)
        ...
        // botón modificar (aparece a la derecha cuando la fila está seleccionada)
        ...
        btnModificar.addEventListener("click", function(e){
            e.stopPropagation();
            // activar edición: llenar campos con los valores actuales
            filaEnEdicion = fila;
        ...
        // click en fila: toggle seleccion
...
// click fuera de la tabla -> deseleccionar filas
...
// icon-blocks (opción B): muestran mensaje
document.getElementById("ic1").addEventListener("click", () => alert("Hiciste clic en Cuadro 1"));
document.getElementById("ic2").addEventListener("click", () => alert("Hiciste clic en Cuadro 2"));
document.getElementById("ic3").addEventListener("click", () => alert("Hiciste clic en Cuadro 3"));

// PDF: usamos html2canvas + jsPDF integrado opcionalmente si quieres
document.getElementById("btnPDF").addEventListener("click", async function(){
    ...
    try {
        ...
        if (pdfHeight <= pdf.internal.pageSize.getHeight() - 40) {
            ...
            // dividir en paginas (misma técnica que usamos antes)
            let remainingHeight = canvas.height;
            const pageCanvas = document.createElement("canvas");
            const pageCtx = pageCanvas.getContext("2d");
            pageCanvas.width = canvas.width;
            const pageHeightPx = Math.floor(canvas.width * ((pdf.internal.pageSize.getHeight()-40) / pdfWidth));
            let position = 0;
            while (remainingHeight > 0) {
                ...
                if (position === 0) pdf.addImage(pageImg, "PNG", 20, 20, pdfWidth, (sliceH * pdfWidth) / pageCanvas.width);
                else { pdf.addPage(); pdf.addImage(pageImg, "PNG", 20, 20, pdfWidth, (sliceH * pdfWidth) / pageCanvas.width); }
